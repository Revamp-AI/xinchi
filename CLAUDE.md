...
- Styling: `app/coss.css` holds the Tailwind v4 `@theme` tokens (light and `.dark` sets) and is imported by `app/globals.css`, which holds the product-specific classes and breakpoints. `components/focus/theme.jsx` wraps the app in next-themes (class attribute, storage key `focus-theme`) and provides the appearance toggle. The product classes in `globals.css` still hard-code light hex colors rather than using the tokens, so dark mode only restyles the Coss primitives until they are migrated.
