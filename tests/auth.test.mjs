...
test('all private reads and writes reject anonymous callers; setup is same-origin and one-time',async()=>{
 for(const p of ['state','sources','sources/manual:private','export','events/private','jobs/private','update-draft'])assert.equal((await route.GET(req(p))).status,401,p);
...
test('Google login fails closed when no workspace owner is configured',()=>{
 const original=process.env.XIN_ALLOWED_EMAIL;delete process.env.XIN_ALLOWED_EMAIL;
 try{assert.equal(auth.allowedEmail(),'');assert.throws(()=>auth.beginGoogle(),/Set XIN_ALLOWED_EMAIL/);}finally{process.env.XIN_ALLOWED_EMAIL=original;}
});
...
test('wrong browser and expired state cannot exchange tokens; callbacks are single-use',async()=>{
 const flow=newFlow();await assert.rejects(()=>auth.finishGoogle('code',flow.state,'other-browser',pair.publicKey),/another browser/);
 const expired=newFlow();auth.authDb.prepare('UPDATE oauth_attempts SET expires_at=0 WHERE state_hash=?').run(digest(expired.state));
 await assert.rejects(()=>auth.finishGoogle('code',expired.state,expired.browser,pair.publicKey),/expired/);
 const result=await complete(flow,{scope:'openid email profile'});assert.equal(result.gmail,false);assert.ok(auth.sessionFor(result.token));auth.endSession(result.token);
...
test('successful Google identity establishes a session and Gmail access together; secrets stay server-side',async()=>{
 const flow=newFlow();const result=await complete(flow);session=result.token;
 assert.equal(result.gmail,true);assert.equal(conn.connectionState().gmail.configured,true);
 ...
 assert.equal(auth.sessionFor(session).email,'owner@example.com');
 const stored=auth.authDb.prepare('SELECT token_hash FROM sessions').all();assert.ok(stored.every(r=>r.token_hash!==session));
...
test('authenticated writes still reject cross-origin calls and logout revokes access',async()=>{
 ...
 assert.equal(auth.sessionFor(session),null);assert.equal((await route.GET(req('export',{token:session}))).status,401);
});
test('the callback route verifies Google keys, creates the cookie, redirects, and retires the browser flow',async()=>{
 const f=newFlow(),old=global.fetch;const publicJwk=await exportJWK(pair.publicKey);
