...
test.beforeEach(async()=>{for(const key of ['gmail_query','gmail_page','gmail_history','gmail_email','granola_page','granola_since','fireflies_page'])await setSetting(key,null);await setSetting('gmail_query','-in:spam -in:trash');await saveSecrets({gmail_tokens:{...fixtureTokens},granola_key:'fixture-granola-secret',fireflies_key:'fixture-fireflies-secret'});});
