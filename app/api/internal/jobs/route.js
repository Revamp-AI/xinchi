...
export async function GET(req){
 ...
 try{const scheduled=await scheduleProviderSyncs();let autoMatch;try{autoMatch=await runAutoContactMatching();}catch{autoMatch={error:'Automatic contact matching will retry.'};}await drainCloudWork((...args)=>start(ingestionWorkflow,args),id=>getRun(id).status);return Response.json({processed:true,scheduled,autoMatch},{headers:{'Cache-Control':'no-store'}});}
