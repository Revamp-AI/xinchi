...
async function applyMerge(target,source,automaticEmail='',recordDecision=true){
 ...
 if(recordDecision){const id=uid();await run("INSERT INTO identity_decisions(id,kind,left_id,right_id,before_snapshot,after_hash) VALUES(?,'merge',?,?,?::jsonb,?)",id,target_id,source_id,JSON.stringify(automaticEmail?{...before,automatic_email:automaticEmail}:before),hash(await snapshot(ids)));return id;}
}
...
export async function undoMerge(id){
 const ids=await transaction(async()=>{await lockWorkspace();const d=await one("SELECT * FROM identity_decisions WHERE id=? AND kind='merge' AND undone_at IS NULL FOR UPDATE",id);if(!d)throw Error('Merge not found or already undone.');const ids=d.before_snapshot.bulk_contact_ids||[d.left_id,d.right_id];if(hash(await snapshot(ids))!==d.after_hash)throw Error('These records changed after the merge. Review them before undoing; no later edits were removed.');
