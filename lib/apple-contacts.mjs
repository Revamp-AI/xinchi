...
export async function importAppleContacts(input,{apply=false}={}){
 ...
 return transaction(async()=>{
  ...
  for(const r of records){
   ...
   const external='apple-contact-'+hash(r.key),sourceId='manual:'+external,content=hash(r);
   // A stable, unchanged card already belongs to its chosen profile. Avoid
   // rescanning the entire address book or rematching it on every reimport.
   ...
   const id=[...found][0],existing=byId.get(id);
   let c;
   if(existing){
