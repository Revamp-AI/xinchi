...
ensureColumn('sync_runs','pid','INTEGER');ensureColumn('sync_runs','updated_at','TEXT');ensureColumn('sync_runs','changed','INTEGER NOT NULL DEFAULT 0');
...
export function ensureColumn(table,column,ddl){if(!db.prepare(`PRAGMA table_info(${table})`).all().some(c=>c.name===column))db.exec(`ALTER TABLE ${table} ADD COLUMN ${column} ${ddl}`);}
...
export function searchSources(q='',provider='',offset=0){
 ...
 return{total:one('SELECT count(*) AS n FROM sources s'+join+clause,...args).n,records:all('SELECT s.id,s.provider,s.title,s.occurred_at,s.coverage,substr(s.body,1,250) AS excerpt FROM sources s'+join+clause+' ORDER BY s.occurred_at DESC LIMIT 40 OFFSET ?',...args,Math.max(0,Number(offset)||0))};
}
...
ensureColumn('items','last_action',"TEXT NOT NULL DEFAULT ''");ensureColumn('items','fallback',"TEXT NOT NULL DEFAULT ''");
const fields=['title','kind','status','done_when','next_action','owner','checkpoint','hard_deadline','dependency','last_action','fallback','evidence','reason','source_id','source_quote','shared'];
...
export function saveItem(input){return transaction(()=>{
 ...
 const row=before?{...before}:{id:uid(),title:'',kind:'action',status:'candidate',done_when:'',next_action:'',owner:'You',checkpoint:'',hard_deadline:'',original_checkpoint:'',dependency:'',last_action:'',fallback:'',evidence:'',reason:'',source_id:null,source_quote:'',shared:0,created_at:stamp(),updated_at:stamp(),version:0};
...
export function dashboard(){return{items:all('SELECT * FROM items ORDER BY created_at'),coverage:all('SELECT provider,coverage,count(*) AS count FROM sources GROUP BY provider,coverage'),jobs:all('SELECT * FROM jobs ORDER BY created_at DESC LIMIT 15').map(x=>({...x,result:x.result_json?JSON.parse(x.result_json):null})),proposals:all("SELECT * FROM proposals WHERE status='pending' ORDER BY created_at DESC").map(x=>({...x,payload:JSON.parse(x.payload)})),sync:all('SELECT * FROM sync_runs s WHERE started_at=(SELECT max(started_at) FROM sync_runs WHERE provider=s.provider) ORDER BY started_at DESC'),focus:getSetting('focus',''),available_hours:getSetting('available_hours',0),sync_history:all('SELECT * FROM sync_runs ORDER BY started_at DESC LIMIT 30'),carryovers:all("SELECT item_id,count(*) AS n FROM events WHERE action='updated' AND json_extract(before_json,'$.checkpoint')<>'' AND json_extract(before_json,'$.checkpoint')<>json_extract(after_json,'$.checkpoint') AND json_extract(after_json,'$.status')='now' GROUP BY item_id")};}
