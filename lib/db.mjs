...
import {SERVERLESS} from './runtime.mjs';
import {queueSourceReview} from './review-batches.mjs';
...
export const dataDir=resolve(/* turbopackIgnore: true */ process.env.XIN_DATA_DIR || (SERVERLESS?'/tmp/focus':resolve(root,'data')));
if(!SERVERLESS)mkdirSync(dataDir,{recursive:true,mode:0o700});
...
export async function upsertSource(doc,raw=doc){return (await transaction(async()=>{
 ...
 if(!['fireflies','granola','gmail','manual','beeper'].includes(provider)||!external_id||!doc.title)throw Error('A source needs a provider, ID, and title.');
 ...
 const rank={metadata:0,empty:0,summary:1,document:2,'email body':2,'message body':2,transcript:3,'deleted upstream':4};
 ...
 if(!old||old.body!==values.body||old.title!==values.title||old.occurred_at!==values.occurred_at)await queueSourceReview(id);

 await run("INSERT INTO contact_queue(source_id,content_hash) VALUES(?,?) ON CONFLICT(source_id) DO UPDATE SET content_hash=excluded.content_hash,state='pending',attempts=0,error='',updated_at=now()",id,content_hash);
...
export async function readSource(id,offset=0,limit=14000){
 const s=await one('SELECT s.*,v.id AS source_version_id FROM sources s LEFT JOIN source_versions v ON v.source_id=s.id AND v.content_hash=s.content_hash WHERE s.id=?',id);if(!s)throw Error('Source not found.');
...
export const exportTables=['sources','source_versions','items','events','settings','jobs','proposals','job_events','sync_runs','review_queue','review_batches',...CONTACT_TABLES,'priority_stacks','priority_order','priorities','priority_updates','priority_suggestions'];
