...
export const db=openDatabase(resolve(dataDir,'xin.sqlite3'));
...
export function ensureColumn(table,column,ddl){transaction(()=>{if(!db.prepare(`PRAGMA table_info(${table})`).all().some(c=>c.name===column))db.exec(`ALTER TABLE ${table} ADD COLUMN ${column} ${ddl}`);});}
...
export function searchSources(q='',provider='',offset=0){
 ...
 const excerpt=tokens.length?"snippet(source_search,2,'"+MARK_START+"','"+MARK_END+"','…',24)":'substr(s.body,1,250)',order=tokens.length?'bm25(source_search),s.occurred_at DESC':'s.occurred_at DESC';
