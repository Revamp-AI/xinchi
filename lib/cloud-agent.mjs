...
export async function runCloudAgent(id,{model=process.env.XIN_AGENT_MODEL||'openai/gpt-6-astra'}={}){
 ...
 try{
  ...
  const result=await agent.generate({prompt:job.prompt,abortSignal:controller.signal});
  ...
  const message=controller.signal.aborted?'The cloud review was interrupted or timed out. Retry with a narrower question.':[401,402,403].includes(error.statusCode)?'Cloud model access is unavailable. Check AI Gateway access and credits.':'The cloud review could not produce verified results. Retry or narrow the question.';
