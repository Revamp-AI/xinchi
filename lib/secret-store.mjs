...
export async function updateSecrets(update){
 if(!POSTGRES_SECRETS){const pending=localUpdate.then(async()=>{const current=localSecrets();const result=await update(current);saveLocalSecrets(current);return result;});localUpdate=pending.catch(()=>{});return pending;}
 return transaction(async()=>{await one("SELECT pg_advisory_xact_lock(hashtext('focus-provider-secrets'))");const current=await secrets(),result=await update(current);await run('INSERT INTO focus_auth.provider_secrets(id,payload) VALUES(1,?::jsonb) ON CONFLICT(id) DO UPDATE SET payload=excluded.payload,updated_at=now()',JSON.stringify(encryptSecrets(current)));return result;});
}
