...
export async function expireLeases(table){
 ...
 return run(`UPDATE ${table} SET ${state}='failed',${message},updated_at=? WHERE ${state} IN ('queued','running') AND ((lease_until IS NOT NULL AND lease_until<now()) OR (lease_until IS NULL AND updated_at::timestamptz<now()-interval '2 minutes'))`,stamp());
}
