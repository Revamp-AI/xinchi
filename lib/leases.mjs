...
export async function expireLeases(table){
 ...
 return run(`UPDATE ${table} SET ${state}='failed',${message},updated_at=? WHERE ${state} IN ('queued','running') ${table==='jobs'?'':`AND NOT EXISTS(SELECT 1 FROM durable_runs d WHERE d.kind='${table==='sync_runs'?'sync':'contacts'}' AND d.run_id=${table}.id)`} AND ((lease_until IS NOT NULL AND lease_until<now()) OR (lease_until IS NULL ${REMOTE_WORKERS?`AND ${state}='running'`:''} AND updated_at::timestamptz<now()-interval '2 minutes'))`,stamp());
}
