...
export function extractInteraction(source,raw,ownerAddresses){
 ...
 if(source.provider==='beeper'){
  const c=raw.chat||{},m=raw.message||{},people=c.participants?.items||[],peers=people.filter(p=>!p.isSelf&&!(m.isSender===true&&p.id===m.senderID)),occurred_at=validTime(m.timestamp);
  ...
  const title=String(c.title||'').trim(),peerTitle=/^(Direct conversation|Beeper contact)$/i.test(title)?'':title;
  // An outgoing sender is the owner. A verified direct-chat title identifies its sole peer.
  // Unresolved participant lists stay in source evidence instead of creating owner/group profiles.
  return{kind:'message',account:c.accountID||'',occurred_at,direction:m.isSender===true?'outbound':m.isSender===false?'inbound':'unknown',qualified:!exclusion,exclusion,thread_key:c.id||'',participants:verified?peers.filter(p=>!p.isNetworkBot).map(p=>({name:String(p.fullName||'').trim()||peerTitle||(m.isSender===false&&m.senderID===p.id?String(m.senderName||'').trim():'')||emailAddress(p.email)||'Beeper contact',address:emailAddress(p.email),external_id:p.id,role:'participant'})):[]};
 }
