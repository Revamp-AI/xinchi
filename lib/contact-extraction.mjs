...
async function bindIdentity(source,person,account){
 ...
 const external=person.address||person.external_id||source.id+':speaker:'+hash(person.name||'unknown').slice(0,20);
 const old=await one('SELECT contact_id FROM contact_identities WHERE provider=? AND account=? AND external_key=?',source.provider,account,external);if(old)return old.contact_id;
 const id=uid(),name=String(person.name||person.address||'Unknown participant').slice(0,200);
 await run('INSERT INTO contacts(id,name,email,confirmed) VALUES(?,?,?,false)',id,name,person.address||'');
 await run('INSERT INTO contact_identities(id,contact_id,provider,account,external_key,display_name,address,source_id) VALUES(?,?,?,?,?,?,?,?)',uid(),id,source.provider,account,external,name,person.address||'',source.id);
 await suggestIdentityMatches(id,name,person.address||'');return id;
}
export async function projectSource(sourceId,guard=async()=>{}){
 const result=await transaction(async()=>{
  ...
  if(event){
   ...
   for(const person of event.participants){const contact=await bindIdentity(source,person,owner||'workspace');contacts.push(contact);await run('INSERT INTO interaction_participants VALUES(?,?,?) ON CONFLICT DO NOTHING',id,contact,person.role);}
