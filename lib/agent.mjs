...
export async function takePendingImportReview(){return (await transaction(async ()=>{
 ...
 const prompt=pending?'Review the source pages assigned by read_import_batch, reconcile them against read_commitments and pending proposals, and propose only useful, source-backed decisions. Every assigned page is provided in the cloud prompt or available through read_import_batch. Be explicit about partial transcripts and remaining coverage; do not claim to have reviewed the whole archive. Older obligations require current evidence. Do not ask the user to retrieve IDs or records available through your tools.':legacy;
