...
import {granolaConnections} from './granola-connections.mjs';
...
export async function scheduleProviderSyncs(){
 ...
 for(const provider of ['granola','fireflies','gmail']){
  if(provider==='gmail'?!configured.gmail_tokens?.refresh_token:provider==='granola'?!granolaConnections(configured).length:!configured[provider+'_key'])continue;
  try{
   const result=await transaction(async()=>{
    ...
    const rescan=provider!=='gmail'&&!await getSetting(provider+'_page')&&(!lastFull||Date.now()-Date.parse(lastFull.finished_at)>=DAY);
