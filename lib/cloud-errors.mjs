import {codexError} from './codex-http.mjs';
...
export function cloudReviewFailure(error,{aborted=false}={}){
 ...
 const subscription=nodes.find(node=>node.name==='FocusCodexError'&&node.provider==='chatgpt');
 ...
 else if(subscription){reason='chatgpt_'+subscription.reason;message=codexError(subscription.reason).message;}
