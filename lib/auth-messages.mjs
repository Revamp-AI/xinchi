...
export const gmailMessages={
 ...
 gmail_rate_limit:'Gmail reached a temporary rate limit. Your connection is saved; Refresh later to resume the import.',
...
export function gmailFailureCode(error){
 ...
 if(error.status===429||['RATE_LIMIT_EXCEEDED','userRateLimitExceeded','rateLimitExceeded'].includes(error.providerReason))return 'gmail_rate_limit';
