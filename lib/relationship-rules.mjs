...
export function relationshipState(contact,interactions,{now=new Date(),coverage={fresh:false,reason:'Coverage has not been verified.'}}={}){
 ...
 const basis={last_meaningful_at:latest?new Date(latest).toISOString():null,age_days:age===null?null:Math.floor(age),cadence_days:contact.cadence_days,coverage,reciprocal,rule_version:RULE_VERSION};
 ...
 if(!contact.confirmed||!contact.tracked)return result('Unclassified',!contact.confirmed?'Confirm this extracted identity.':'Choose a cadence to track this relationship.');
 ...
 if(!coverage.fresh)return result('Unclassified','Coverage is incomplete or stale; silence cannot establish cooling.');
 if(age<=3*contact.cadence_days)return result('Cooling','The last meaningful exchange is beyond the chosen cadence.');
 return result('Dormant','The last meaningful exchange is beyond three cadence periods.');
}
