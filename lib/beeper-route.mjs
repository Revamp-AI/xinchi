...
export async function handleBeeperRequest(req,action,schedule=()=>{}){
 try{
  ...
  // Pairing includes the complete conversation selection; message batches stay small.
  const id=action==='pair'?null:await requireBeeperDevice(req),limit=action==='pair'?4*1024*1024:512*1024,tooLarge=action==='pair'?'Conversation selection exceeds 4 MiB. Run setup --choose to select fewer conversations.':'Batch is too large.';
  if(Number(req.headers.get('content-length'))>limit)return respond({error:tooLarge},413);
  const reader=req.body?.getReader();let bytes=0,chunks=[];if(reader)for(;;){const {done,value}=await reader.read();if(done)break;bytes+=value.length;if(bytes>limit){await reader.cancel();return respond({error:tooLarge},413);}chunks.push(value);}
