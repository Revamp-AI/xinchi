...
import {APP_ORIGIN,HOSTED} from './runtime.mjs';
export {APP_ORIGIN} from './runtime.mjs';
export const CALLBACK=APP_ORIGIN+'/api/auth/google/callback';
export const SESSION_COOKIE=HOSTED?'__Host-xin_session':'xin_session';
export const FLOW_COOKIE=HOSTED?'__Host-xin_google_flow':'xin_google_flow';
...
export function cookie(name,value,age){return `${name}=${value}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${age}${HOSTED?'; Secure':''}`;}
...
export function checkLocalRequest(req,write=false){
 if(req.headers.get('host')!==new URL(APP_ORIGIN).host)throw Object.assign(Error('Open Focus at its configured address.'),{status:403});
...
export async function finishGoogle(code,state,browser,keys=googleKeys){
 ...
 catch(error){if(error.authCode==='account')throw error;gmailIssue=await recordGmailIssue(error);(await recordAuthEvent(stage,'warning',gmailIssue,error));}
 stage='session';
 const token=random();
