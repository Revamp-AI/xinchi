...
export async function evaluateContacts(ids=null){
 if(!ids){
  const last=await getSetting('contacts_last_evaluation');
  // A deployment must refresh existing classifications even inside the normal cache window.
  if(last&&Date.now()-Date.parse(last)<900000&&!await one('SELECT c.id FROM contacts c LEFT JOIN contact_states s ON s.contact_id=c.id WHERE c.merged_into IS NULL AND c.archived=false AND s.rule_version IS DISTINCT FROM ? LIMIT 1',RULE_VERSION))return;
 }
...
export async function contactDetail(id,evaluate=true){
 ...
 return{...contact,relationship:await one('SELECT * FROM contact_states WHERE contact_id=?',id),identities:await all("SELECT i.*,CASE WHEN i.provider='linkedin' THEN s.url END AS profile_url,CASE WHEN i.provider='linkedin' THEN v.raw_json::jsonb #>> '{linkedin_connection,connected_on}' END AS connected_on,v.id AS source_version_id FROM contact_identities i LEFT JOIN sources s ON s.id=i.source_id LEFT JOIN source_versions v ON v.source_id=s.id AND v.content_hash=s.content_hash WHERE i.contact_id=?",id),affiliations:await all('SELECT a.*,o.name AS organization FROM affiliations a JOIN organizations o ON o.id=a.organization_id WHERE contact_id=? ORDER BY ended_on NULLS FIRST,started_on DESC NULLS LAST',id),interactions:await readInteractions(id),history:await all('SELECT * FROM contact_state_history WHERE contact_id=? ORDER BY changed_at DESC LIMIT 40',id),items:await all('SELECT i.* FROM items i JOIN item_contacts c ON c.item_id=i.id WHERE c.contact_id=? ORDER BY i.created_at DESC',id),drafts:await all('SELECT * FROM message_drafts WHERE contact_id=? ORDER BY created_at DESC LIMIT 20',id),profile_source_id:'manual:contact-profile-'+id};
}
