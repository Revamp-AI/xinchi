...
export function emailAddress(input){const parsed=addresses.parseOneAddress(String(input||'').trim());return parsed?.type==='mailbox'?parsed.address.toLowerCase():'';}
...
export async function importContactsCsv(csv){
 ...
 let added=0;await transaction(async()=>{await lockWorkspace();for(const r of records){const key=hash(r);if(await one("SELECT id FROM contact_identities WHERE provider='csv' AND account='workspace' AND external_key=?",key))continue;const c=await saveContact(r);await run("INSERT INTO contact_identities(id,contact_id,provider,account,external_key,display_name,address) VALUES(?,?,'csv','workspace',?,?,?)",uid(),c.id,key,c.name,c.email);await suggestIdentityMatches(c.id,c.name,c.email);added++;}});return{total:records.length,added};
}
...
export async function listContacts({q='',segment='',tag='',archived=false,offset=0,evaluate=true}={}){
 if(evaluate)await evaluateContacts();const where=['c.merged_into IS NULL','c.archived=?'],args=[archived===true||archived==='true'];
 if(q){where.push("(c.name ILIKE ? OR c.email ILIKE ? OR c.role ILIKE ? OR EXISTS(SELECT 1 FROM affiliations a JOIN organizations o ON o.id=a.organization_id WHERE a.contact_id=c.id AND o.name ILIKE ?))");args.push(...Array(4).fill('%'+text(q,200)+'%'));}
