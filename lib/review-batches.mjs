...
export async function queueSourceReview(id,{force=false}={}){
 ...
 await run(`INSERT INTO review_queue(source_id,source_version_id) VALUES(?,?) ON CONFLICT(source_id) DO UPDATE SET source_version_id=excluded.source_version_id,next_offset=0,job_id=NULL,revisit=false,completed=false,attempts=0,retry_after=NULL,updated_at=now() WHERE review_queue.source_version_id<>excluded.source_version_id`,id,source.version_id);
}
...
export async function completeReviewBatch(jobId){
 ...
 for(const page of batch?.pages||[])await run('UPDATE review_queue SET next_offset=CASE WHEN revisit THEN 0 ELSE ? END,completed=CASE WHEN revisit THEN false ELSE ? END,revisit=false,job_id=NULL,attempts=0,retry_after=NULL,updated_at=now() WHERE source_id=? AND source_version_id=? AND job_id=?',page.end,page.end>=page.total_characters,page.source_id,page.source_version_id,jobId);
}
