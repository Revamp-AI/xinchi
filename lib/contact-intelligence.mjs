...
export async function contactEvidence(contact){
 ...
 return{contact_id:contact.id,name:contact.name,role:contact.role,priority:contact.priority,manual_purpose_tags:contact.tags,notes:contact.notes.slice(0,3000),paused:contact.paused,do_not_contact:contact.do_not_contact,snoozed_until:contact.snoozed_until,
...
export async function runContactIntelligence({generate=generateContactInsights,force=false}={}){
 ...
 try{
  ...
  const batch=pending.filter(c=>(c.has_conversations||c.notes.trim())&&!deferred.has(c.id)).sort((a,b)=>Number(b.priority)-Number(a.priority)||Number(b.tracked)-Number(a.tracked)||(b.latest||'').localeCompare(a.latest||'')||a.id.localeCompare(b.id)).slice(0,8);
