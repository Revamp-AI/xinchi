...
export async function request(url,options={},attempt=0){const response=await fetch(url,{...options,signal:AbortSignal.timeout(60000)});if([429,500,502,503,504].includes(response.status)&&attempt<3){await new Promise(r=>setTimeout(r,Math.min(10000,1000*2**attempt)));return request(url,options,attempt+1);}let value;try{value=await response.json();}catch{throw Error('The provider returned an unreadable response.');}if(!response.ok){const e=Error('Provider request failed ('+response.status+'). '+(value.error?.message||value.message||'Check the connection and retry.'));e.status=response.status;e.providerCode=typeof value.error==='string'?value.error:undefined;e.providerReason=value.error?.details?.find(d=>d['@type']==='type.googleapis.com/google.rpc.ErrorInfo')?.reason||value.error?.errors?.[0]?.reason;throw e;}return value;}
