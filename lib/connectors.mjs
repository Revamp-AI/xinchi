...
import {dataDir,root,stamp,uid,all,one,run,getSetting,setSetting,upsertSource} from './db.mjs';
...
export function connectionState(){const s=secrets();return{fireflies:{configured:!!s.fireflies_key},granola:{configured:!!s.granola_key},gmail:{configured:!!s.gmail_tokens?.refresh_token,client_ready:!!s.google_client?.client_id,email:getSetting('gmail_email',''),query:getSetting('gmail_query','-in:spam -in:trash'),issue:s.gmail_issue?.code&&gmailMessages[s.gmail_issue.code]?{code:s.gmail_issue.code,message:gmailMessages[s.gmail_issue.code]}:null}};}
...
export async function request(url,options={},attempt=0){
 ...
 const response=await fetch(url,{...fetchOptions,signal:AbortSignal.timeout(60000)});
 ...
 if((rateLimited||[500,502,503,504].includes(response.status))&&attempt<(rateLimited?6:3)){
  ...
  onRetry?.({delay,rateLimited});await new Promise(r=>setTimeout(r,delay));return request(url,options,attempt+1);
 }
...
export function clearGmailIssue(){const s=secrets();delete s.gmail_issue;saveSecrets(s);}
export function recordGmailIssue(error){const s=secrets();s.gmail_issue={code:gmailFailureCode(error),at:stamp()};saveSecrets(s);return s.gmail_issue.code;}
export async function saveGoogleGrant(tokens,identity){
 ...
 try{profile=await gmailGet('profile',{},tokens.access_token);}
  ...
  const current=secrets();current.gmail_tokens=grant;current.gmail_issue={code:gmailFailureCode(error),at:stamp()};saveSecrets(current);setSetting('gmail_email',identity.email);throw error;
 }
 ...
 const current=secrets();current.gmail_tokens=grant;delete current.gmail_issue;saveSecrets(current);setSetting('gmail_email',profile.emailAddress);return true;
}
...
export function staleRun(row,nowMs){if(!isProcessAlive(row.pid))return true;const seen=Math.max(Date.parse(row.updated_at||'')||0,Date.parse(row.started_at||'')||0);return nowMs-seen>15*60*1000;}
export function recoverSyncRuns(){for(const r of all("SELECT * FROM sync_runs WHERE state='running'"))if(staleRun(r,Date.now())){if(isProcessAlive(r.pid))try{process.kill(r.pid,'SIGTERM');}catch(e){if(e.code!=='ESRCH'&&e.code!=='EPERM')throw e;}run("UPDATE sync_runs SET state='failed',finished_at=?,message='Interrupted; retry the import' WHERE id=?",stamp(),r.id);}}
export function startSync(provider){if(!['fireflies','granola','gmail'].includes(provider))throw Error('Unknown provider.');if(!connectionState()[provider].configured)throw Error('Set up this connection first.');recoverSyncRuns();if(one("SELECT id FROM sync_runs WHERE state='running'"))throw Error('A source import is already running.');const id=uid();run('INSERT INTO sync_runs(id,provider,started_at,state) VALUES(?,?,?,?)',id,provider,stamp(),'running');const child=spawn(process.execPath,[resolve(root,'scripts/sync-worker.mjs'),id],{cwd:root,detached:true,stdio:'ignore',env:{...process.env,XIN_APP_ROOT:root}});child.unref();run('UPDATE sync_runs SET pid=?,updated_at=? WHERE id=?',child.pid,stamp(),id);return{id};}
export async function syncProvider(provider,onProgress=()=>{},onStatus=()=>{}){
 ...
 if(provider==='gmail'){
  ...
  const getMessage=async id=>{try{const m=await readGmail('messages/'+encodeURIComponent(id),{format:'full'});if((m.labelIds||[]).some(l=>['SPAM','TRASH'].includes(l)))return;put(normalizeGmail(m),m);}catch(e){if(e.status!==404)throw e;const old=one('SELECT * FROM sources WHERE id=?','gmail:'+id);if(old)put({...old,coverage:'deleted upstream',body:'This message was deleted upstream. Prior snapshots remain in the archive.'},{deleted:true,id});}};
  ...
  let pending=getSetting('gmail_page');if(!pending){const profile=await readGmail('profile');pending={pageToken:null,historyId:profile.historyId,query};setSetting('gmail_email',profile.emailAddress);}
  for(let p=0;p<10;p++){const list=await readGmail('messages',{q:query,maxResults:100,pageToken:pending.pageToken});for(const m of list.messages||[])await getMessage(m.id);if(!list.nextPageToken){setSetting('gmail_history',pending.historyId);setSetting('gmail_page',null);return{changed,total,complete:true};}pending.pageToken=list.nextPageToken;setSetting('gmail_page',pending);}
