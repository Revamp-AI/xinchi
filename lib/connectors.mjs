...
export function recoverSyncRuns(){for(const r of all("SELECT * FROM sync_runs WHERE state='running'"))if(staleRun(r,Date.now()))run("UPDATE sync_runs SET state='failed',finished_at=?,message='Interrupted; retry the import' WHERE id=?",stamp(),r.id);}
