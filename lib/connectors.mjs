...
import {beeperState} from './beeper.mjs';
...
import {granolaConnections,configureGranola,nextGranolaWorkspace} from './granola-connections.mjs';
export {secrets,saveSecrets} from './secret-store.mjs';
export async function connectionState(){const s=await secrets();return{beeper:await beeperState(),fireflies:{configured:!!s.fireflies_key},granola:{configured:!!granolaConnections(s).length,workspaces:granolaConnections(s).map(({id,label})=>({id,label}))},gmail:{configured:!!s.gmail_tokens?.refresh_token,client_ready:!!googleClient()?.client_id,email:(await getSetting('gmail_email','')),query:(await getSetting('gmail_query','-in:spam -in:trash')),issue:s.gmail_issue?.code&&gmailMessages[s.gmail_issue.code]?{code:s.gmail_issue.code,message:gmailMessages[s.gmail_issue.code]}:null}};}
export async function configure(input){
 await updateSecrets(s=>{if(input.fireflies_key)s.fireflies_key=String(input.fireflies_key).trim();configureGranola(s,input);});
...
export async function startSync(provider,{rescan=false}={}){
 if(!['fireflies','granola','gmail'].includes(provider))throw Error('Unknown provider.');
 if(typeof rescan!=='boolean'||(rescan&&!['fireflies','granola'].includes(provider)))throw Error('History rescans are available only for Fireflies and Granola.');
 ...
 if(await one("SELECT id FROM sync_runs WHERE provider=? AND state IN ('queued','running','uploading')",provider))throw Error('An import for this connection is already running.');
 if(rescan)await setSetting(provider+'_page',await (provider==='fireflies'?firefliesWindow:granolaWindow)({rescan:true}));
...
export async function syncProvider(provider,onProgress=()=>{},onStatus=()=>{},guard=async()=>{}){
 ...
 if(provider==='fireflies'){
  ...
  const pending=await firefliesWindow(),{boundary,since,participant}=pending;let skip=pending.skip;
  await checkpoint('fireflies_page',pending);
  for(let page=0;page<200;page++){
   const value=await request('https://api.fireflies.ai/graphql',{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+s.fireflies_key},body:JSON.stringify({query:'query($skip:Int,$since:DateTime,$until:DateTime,$participants:[String!]){transcripts(limit:50,skip:$skip,fromDate:$since,toDate:$until,participants:$participants){id title dateString transcript_url participants sentences{speaker_name text start_time end_time} summary{short_summary action_items overview}}}',variables:{skip,since,until:boundary,participants:[participant]}})});
   ...
   for(const r of rows){const summary=[r.summary?.short_summary,r.summary?.overview,r.summary?.action_items].filter(Boolean).join('\n\n');const transcript=(r.sentences||[]).map(x=>`[${x.start_time}s] ${x.speaker_name||'Unattributed'}: ${x.text}`).join('\n');(await put({provider,external_id:r.id,title:r.title||'Untitled meeting',occurred_at:r.dateString||'',url:r.transcript_url||'',body:[summary,transcript?'Transcript\n'+transcript:''].filter(Boolean).join('\n\n'),coverage:transcript?'transcript':summary?'summary':'metadata'},r));skip++;await checkpoint('fireflies_page',firefliesCheckpoint({...pending,skip}));}
   if(rows.length<50){await transaction(async()=>{await guard();for(const [key,value]of Object.entries(await firefliesCompletedSettings(pending)))await setSetting(key,value);});return{changed,total,complete:true};}
  }return{changed,total,complete:false,note:'Reached the per-run safety limit; more history may remain.'};
 }
 if(provider==='granola'){
  const connections=granolaConnections(s);if(!connections.length)throw Error('Add your Granola Personal API key.');const pending=await granolaWindow();let cursor=pending.cursor;
  await checkpoint('granola_page',granolaCheckpoint(pending));
  for(let page=0;page<400;page++){
   const connection=connections.find(c=>c.id===pending.workspaceId);if(!connection)throw Error('Reconnect the saved Granola workspace.');
   const headers={Authorization:'Bearer '+connection.key},since=pending.since;
   ...
   for(const n of await granolaNotesToFetch(list.notes,pending)){try{const note=await request(`https://public-api.granola.ai/v1/notes/${encodeURIComponent(n.id)}`,{headers});let parts=[],tcursor=null;
   ...
   if(!list.hasMore){if(nextGranolaWorkspace(pending)){cursor=null;await checkpoint('granola_page',granolaCheckpoint(pending));continue;}await transaction(async()=>{await guard();for(const [key,value]of Object.entries(await granolaCompletedSettings(pending)))await setSetting(key,value);});return{changed,total,complete:true};}if(!list.cursor||list.cursor===cursor)throw Error('Granola omitted or repeated its next cursor.');cursor=list.cursor;pending.cursor=cursor;pending.pageToken=cursor;await checkpoint('granola_page',granolaCheckpoint(pending));
