...
import {granolaWindow,granolaCompletedSettings,granolaNotesToFetch} from './granola-sync.mjs';
export {secrets,saveSecrets} from './secret-store.mjs';
export async function connectionState(){const s=await secrets();return{beeper:await beeperState(),fireflies:{configured:!!s.fireflies_key},granola:{configured:!!s.granola_key},gmail:{configured:!!s.gmail_tokens?.refresh_token,client_ready:!!googleClient()?.client_id,email:(await getSetting('gmail_email','')),query:(await getSetting('gmail_query','-in:spam -in:trash')),issue:s.gmail_issue?.code&&gmailMessages[s.gmail_issue.code]?{code:s.gmail_issue.code,message:gmailMessages[s.gmail_issue.code]}:null}};}
...
export async function syncProvider(provider,onProgress=()=>{},onStatus=()=>{},guard=async()=>{}){
 ...
 if(provider==='granola'){
  if(!s.granola_key)throw Error('Add your Granola Personal API key.');const headers={Authorization:'Bearer '+s.granola_key};const pending=await granolaWindow();let cursor=pending.cursor;const {since,started,rescan}=pending;
  await checkpoint('granola_page',{cursor,since,started,rescan});
  for(let page=0;page<400;page++){
   ...
   if(!list.hasMore){await transaction(async()=>{await guard();for(const [key,value]of Object.entries(await granolaCompletedSettings(pending)))await setSetting(key,value);});return{changed,total,complete:true};}if(!list.cursor)throw Error('Granola omitted its next cursor.');cursor=list.cursor;await checkpoint('granola_page',{cursor,since,started,rescan});
