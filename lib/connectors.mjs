...
export async function connectionState(){const s=secrets();return{fireflies:{configured:!!s.fireflies_key},granola:{configured:!!s.granola_key},gmail:{configured:!!s.gmail_tokens?.refresh_token,client_ready:!!s.google_client?.client_id,email:(await getSetting('gmail_email','')),query:(await getSetting('gmail_query','-in:spam -in:trash')),issue:s.gmail_issue?.code&&gmailMessages[s.gmail_issue.code]?{code:s.gmail_issue.code,message:gmailMessages[s.gmail_issue.code]}:null}};}
export async function configure(input){const s=secrets();
 ...
 saveSecrets(s);return (await connectionState());
}
export async function request(url,options={},attempt=0){
 ...
 if((rateLimited||[500,502,503,504].includes(response.status))&&attempt<(rateLimited?6:3)){
  ...
  await onRetry?.({delay,rateLimited});await new Promise(r=>setTimeout(r,delay));return request(url,options,attempt+1);
 }
...
export async function saveGoogleGrant(tokens,identity){
 ...
 try{profile=await gmailGet('profile',{},tokens.access_token);}
  ...
  const current=secrets();current.gmail_tokens=grant;current.gmail_issue={code:gmailFailureCode(error),at:stamp()};saveSecrets(current);(await setSetting('gmail_email',identity.email));throw error;
 }
 ...
 const current=secrets();current.gmail_tokens=grant;delete current.gmail_issue;saveSecrets(current);(await setSetting('gmail_email',profile.emailAddress));return true;
}
...
export async function startSync(provider){
 ...
 if(await one("SELECT id FROM sync_runs WHERE state IN ('queued','running')"))throw Error('A source import is already running.');
...
export async function syncProvider(provider,onProgress=()=>{},onStatus=()=>{},guard=async()=>{}){
 ...
 const put=async(doc,raw)=>{await transaction(async()=>{await guard();if((await upsertSource(doc,raw)).changed)changed++;});total++;await onProgress(total);};
 ...
 if(provider==='granola'){
  if(!s.granola_key)throw Error('Add your Granola Personal API key.');const headers={Authorization:'Bearer '+s.granola_key};let cursor=null;const since=(await getSetting('granola_since'));const started=stamp();
  for(let page=0;page<400;page++){
   ...
   if(!list.hasMore){(await checkpoint('granola_since',new Date(Date.parse(started)-60000).toISOString()));return{changed,total,complete:true};}if(!list.cursor)throw Error('Granola omitted its next cursor.');cursor=list.cursor;
