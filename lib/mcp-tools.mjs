...
write('refresh_connection','Start an import from an already configured provider, or request a Beeper companion sync. Returns immediately; poll list_imports.',{provider:z.enum(['gmail','fireflies','granola','beeper'])},async a=>{background();return a.provider==='beeper'?requestBeeperSync():startSync(a.provider);},{dispatch:true,external:true,destructive:false});
