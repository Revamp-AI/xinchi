...
export function importResearchArchive(archive,pasted=''){
 const docs=new Map();
 ...
 let changed=0;for(const d of docs.values()){const {raw,...doc}=d;if(upsertSource(doc,raw).changed)changed++;}return{total:docs.size,changed};
}
export function importDocuments(docs){if(!Array.isArray(docs)||docs.length>5000)throw Error('Import an array of up to 5,000 source records.');let changed=0;for(const d of docs)if(upsertSource(d,d.raw||d).changed)changed++;return{total:docs.length,changed};}
