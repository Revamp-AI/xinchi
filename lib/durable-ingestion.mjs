...
export async function advanceDurableUnit(kind,id,token,workflowId,revision,adapters={}){
 ...
 try{
  ...
  return transaction(async()=>{
   ...
   const outcome={revision:next,done:failed,...(!failed?{waitMs}:{failed:true,permanent:Boolean(error.permanent)})};
...
// Replace an abandoned owner, never its saved cursor. Late callbacks from the
// previous workflow cannot claim or commit a unit after its tokens are cleared.
export async function recoverStalledRun(kind,id,workflowId,revision,{terminal=false}={}){
 return transaction(async()=>{await lockWorkspace();
  const row=await one('SELECT * FROM durable_runs WHERE kind=? AND run_id=? FOR UPDATE',kind,id);
  if(!row||row.completed||row.workflow_id!==workflowId||row.revision!==revision)return false;
  const now=Date.now(),wait=Math.max(15*60000,Number(row.outcome?.waitMs||0)+5*60000);
  if(row.unit_until&&Date.parse(row.unit_until)>now||!terminal&&now-Date.parse(row.updated_at)<wait)return false;
  const table=tableFor(kind),parent=await one('SELECT state FROM '+table+' WHERE id=? FOR UPDATE',id);
  if(!parent||!['queued','running'].includes(parent.state))return false;
  await run("UPDATE durable_runs SET workflow_id='',dispatch_token='',dispatch_until=NULL,unit_token='',unit_until=NULL,outcome=?::jsonb,updated_at=now() WHERE kind=? AND run_id=?",JSON.stringify({done:false,revision:row.revision}),kind,id);
  await run("UPDATE "+table+" SET state='queued',lease_owner='',lease_until=NULL,updated_at=?,message='Recovering interrupted import from saved progress' WHERE id=?",stamp(),id);
  return true;
 });
}

// A large archive spans multiple workflow runs while retaining the same cursor.
export async function yieldDurableRun(kind,id,token,workflowId){
 return transaction(async()=>{await lockWorkspace();const row=await one('SELECT * FROM durable_runs WHERE kind=? AND run_id=? FOR UPDATE',kind,id);
  if(!row||row.completed||row.dispatch_token!==token||row.workflow_id!==workflowId||row.unit_until&&Date.parse(row.unit_until)>Date.now())return false;
  await run("UPDATE durable_runs SET workflow_id='',dispatch_token='',dispatch_until=NULL,updated_at=now() WHERE kind=? AND run_id=?",kind,id);
  await run("UPDATE "+tableFor(kind)+" SET state='queued',lease_owner='',lease_until=NULL,updated_at=?,message='Saved progress; continuing in the next background run' WHERE id=?",stamp(),id);return true;
 });
}
