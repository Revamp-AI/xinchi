...
export async function prepareDispatches(){
 return transaction(async()=>{
  await lockWorkspace();
  await run("UPDATE sync_runs SET state='failed',message='Upload expired; choose the file again',finished_at=?,updated_at=? WHERE state='uploading' AND updated_at::timestamptz<now()-interval '24 hours'",stamp(),stamp());
...
export async function resumeDurable(kind,id,provider){
 return transaction(async()=>{
  ...
  if(await one("SELECT id FROM "+table+" WHERE state IN ('queued','running','uploading')"))throw Error('An import is already running.');
...
async function claimUnit(kind,id,token,workflowId,revision){
 ...
 return transaction(async()=>{
  ...
  if(row.completed||!parent||!['queued','running'].includes(parent.state))return{result:{...summary(row),done:true}};
  ...
  await run("UPDATE "+table+" SET state='running',lease_owner='',lease_until=NULL,pid=NULL,updated_at=?,message='Import continues automatically in the background' WHERE id=?",stamp(),id);
