...
export async function initialProviderCursor(provider){
 ...
 if(provider==='granola'){
  const saved=await getSetting('granola_page');
  return{...base,phase:'list',pageToken:token(saved?.cursor,provider),since:saved?saved.since:await getSetting('granola_since'),started:saved?.started||stamp(),pendingIds:[],nextPageToken:null};
 }
 const participant=(process.env.XIN_FIREFLIES_PARTICIPANT_EMAIL||process.env.XIN_ALLOWED_EMAIL||'').trim();
 if(!participant)throw failure(provider,'configuration');
 const saved=await getSetting('fireflies_page'),pending=saved?.participant===participant?saved:null;
 const skip=Number(pending?.skip||0);if(!Number.isSafeInteger(skip)||skip<0)throw failure(provider,'cursor');
 return{...base,phase:'record',boundary:pending?.boundary||stamp(),skip,participant};
}
...
function granolaPage(cursor,records=[]){
 ...
 if(cursor.nextPageToken){cursor.pageToken=cursor.nextPageToken;cursor.nextPageToken=null;cursor.phase='list';return outcome(cursor,records,{granola_page:{cursor:cursor.pageToken,since:cursor.since,started:cursor.started}});}
 cursor.phase='done';return outcome(cursor,records,{granola_since:new Date(Date.parse(cursor.started)-60000).toISOString(),granola_page:null});
}
async function advanceGranola(cursor){
 ...
 if(cursor.phase==='list'){
  ...
  cursor.pendingIds=ids(list.notes,'granola',30);if(typeof list.hasMore!=='boolean')throw failure('granola','response');
 ...
 if(cursor.phase==='detail'&&cursor.pendingIds?.length){
  const note=await get('notes/'+encodeURIComponent(cursor.pendingIds[0]));if(note.id!==cursor.pendingIds[0])throw failure('granola','response');
 ...
 if(cursor.phase==='transcript'&&cursor.note){
  const page=await get('notes/'+encodeURIComponent(cursor.note.id)+'/transcript',{page_size:50,cursor:cursor.transcriptToken});
...
async function advanceFireflies(cursor){
 if(cursor.phase!=='record'||!Number.isSafeInteger(cursor.skip)||cursor.skip<0||!cursor.participant||!Number.isFinite(Date.parse(cursor.boundary)))throw failure('fireflies','cursor');
 const stored=await secrets();if(!stored.fireflies_key)throw failure('fireflies','configuration');
 const value=await request('fireflies','https://api.fireflies.ai/graphql',{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+stored.fireflies_key},body:JSON.stringify({query:'query($skip:Int,$until:DateTime,$participants:[String]){transcripts(limit:1,skip:$skip,toDate:$until,participants:$participants){id title dateString transcript_url participants sentences{speaker_name text start_time end_time} summary{short_summary action_items overview}}}',variables:{skip:cursor.skip,until:cursor.boundary,participants:[cursor.participant]}})});
 const rows=value.data?.transcripts;ids(rows,'fireflies',1);
 if(!rows.length){cursor.phase='done';return outcome(cursor,[],{fireflies_page:null});}
 ...
 cursor.skip++;return outcome(cursor,[record],{fireflies_page:{boundary:cursor.boundary,skip:cursor.skip,participant:cursor.participant}});
}
