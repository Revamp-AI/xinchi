...
import {checkJobBudget} from '../lib/job-budget.mjs';
export async function runContactWorker(id){
 const claim=await claimLease('contact_runs',id);if(!claim)return;const {token}=claim;let total=claim.row.imported||0,checking=false;
 ...
 try{for(;;){checkJobBudget();await transaction(guard);const q=await one("SELECT * FROM contact_queue WHERE state='pending' ORDER BY updated_at,source_id LIMIT 1");if(!q)break;
 ...
 }catch(error){if(error.yieldJob)await run("UPDATE contact_runs SET state='queued',lease_owner='',lease_until=NULL,pid=NULL,updated_at=?,message='Continuing extraction in the next cloud invocation' WHERE id=? AND state='running' AND lease_owner=? AND lease_until>now()",stamp(),id,token);else await run("UPDATE contact_runs SET state='failed',finished_at=?,message='Extraction interrupted; retry to continue' WHERE id=? AND state='running' AND lease_owner=?",stamp(),id,token);}finally{clearInterval(beat);}
}
