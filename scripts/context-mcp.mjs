import {listContacts,contactDetail,readInteractions,contactStatus} from '../lib/contacts.mjs';
...
for await(const line of readline.createInterface({input:process.stdin})){
 ...
 if(m.method==='initialize')reply(m.id,{protocolVersion:'2024-11-05',capabilities:{tools:{}},serverInfo:{name:'Focus stored context',version:'1.0'}});
  ...
  if(process.env.XIN_JOB_ID){const label=name==='read_source'?'Reading: '+(result.title||a.id):name==='search_context'?'Searching context: '+(a.query||'recent material'):'Checking your commitments and decisions';await run("INSERT INTO job_events(job_id,label,created_at) SELECT id,?,? FROM jobs WHERE id=? AND status='running' AND lease_until>now()",label,stamp(),process.env.XIN_JOB_ID);await run("UPDATE jobs SET progress=?,updated_at=? WHERE id=? AND status='running' AND lease_until>now()",label,stamp(),process.env.XIN_JOB_ID);}
